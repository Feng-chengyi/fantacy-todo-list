...
export function buildListGroups(
  ...
  const push = (key: string, occ: Occurrence): void => {
  ...
  for (const task of tasks) {
    if (task.date == null) {
      ...
      push('inbox', { task, date: '', status: task.status })
      continue
    }
    ...
    if (task.repeat) {
      ...
      for (const entry of entries) {
        ...
        push(entry.date, { task, date: entry.date, status: entry.status as TaskStatus })
      }
    } else {
      if (!matchesFilter(task.status, filter)) continue
      push(task.date, { task, date: task.date, status: task.status })
    }
  }
  ...
  const dateKeys = [...map.keys()].filter((k) => k !== 'inbox').sort()
